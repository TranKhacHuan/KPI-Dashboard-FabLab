...
export default function ManagerView({ path }: { path: string }) {
  console.log("Rendering ManagerView, path:", path);
  const { tasks, users, currentUserId } = useAppStore();
  const me = users.find((u) => u.id === currentUserId)!;
  const [year, setYear] = useState<number>(new Date().getFullYear());
  const [quarter, setQuarter] = useState<string>("all");

  const allTasksYear = useMemo(() => 
    tasks.filter((t) => t.nam === year), 
    [tasks, year]
  );
  const filteredByQ = useMemo(
    () => (quarter === "all" ? allTasksYear : allTasksYear.filter((t) => String(t.quy) === quarter)),
    [allTasksYear, quarter]
  );
  ...
  }, [users, me.department, filteredByQ]);
  ...
  const perQuarter = useMemo(() => {
    const data = [1, 2, 3, 4].map((q) => {
      ...
      return { quarter: `Q${q}`, total: qt.length, done, rate: qt.length ? Math.round((done / qt.length) * 100) : 0 };
    });
    return data;
  }, [allTasksYear]);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-accent">Manager · Tất cả phòng ban</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Hoàn thành" value={totals.completed} icon={CheckCircle2} tone="success" hint={`${completionRate}% tỷ lệ`} />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="p-6">
          <div className="flex items-baseline justify-between mb-4">
            <div>
              <h2 className="font-display text-xl font-semibold">Tỷ lệ hoàn thành theo nhân viên</h2>
              <p className="text-xs text-muted-foreground">{quarter === "all" ? "Cả năm" : `Quý ${quarter}`} · {year}</p>
            </div>
          </div>
              ...
              <div key={m.id}>
                <div className="flex items-center justify-between mb-1.5 text-sm">
                  ...
                  <span className="text-muted-foreground">
                    {m.done}/{m.total} · <span className="font-semibold text-foreground">{m.rate}%</span>
                  </span>
                </div>
                <Progress value={m.rate} className="h-2" />
              </div>
            ))}
            {perMember.length === 0 && <p className="text-muted-foreground text-sm">Chưa có dữ liệu.</p>}
          </div>
        </Card>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={perQuarter} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number, _n, p: any) => [`${v}% (${p.payload.done}/${p.payload.total})`, "Hoàn thành"]}
                />
      ...
      <Card className="p-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredByQ.map((t) => {
                const a = users.find((u) => u.id === t.nguoiThucHien);
                return (
                  <tr key={t.maCV} className="border-b border-border/50 hover:bg-muted/40 transition-smooth">
                    ...
                    <td className="py-3 pr-4 text-muted-foreground whitespace-nowrap">{format(new Date(t.ngayMucTieu), "dd/MM/yyyy")}</td>
                    <td className="py-3 pr-4 whitespace-nowrap">
                      <Badge variant="outline" className={`text-[10px] ${statusTone[t.trangThai]}`}>{statusLabel[t.trangThai]}</Badge>
                    </td>
                  </tr>
                );
              })}
              {filteredByQ.length === 0 && (
                <tr><td colSpan={7} className="py-12 text-center text-muted-foreground">Không có dữ liệu.</td></tr>
              )}
            </tbody>
          </table>
        </div>
